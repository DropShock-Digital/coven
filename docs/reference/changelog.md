...
- **Double keystrokes fixed on Windows TUI.** `coven tui` and the session browser now only react to key-press events on Windows, eliminating the duplicate input bug where typing `a` produced `aa`, arrow keys jumped two rows, and Enter activated twice. See [Coven TUI](/start/coven-tui) and [Getting started](/GETTING-STARTED).
...
- **JSON sessions output.** `coven sessions --json` emits machine-readable session listings for scripts, dashboards, and external clients. See [Comux demo loop](/COMUX-DEMO-LOOP).
...
- **OpenCoven positioning and brand.** Refreshed product copy across the docs and CLI to frame Coven as an ecosystem for persistent AI familiars, with updated brand tokens and design guidance.
- **Refined brand palette.** Updated the OpenCoven palette to a muted lavender-grey (`#9A8ECD`) with a new complementary accent system and dedicated dark- and light-mode surface tokens. Existing legacy color aliases are preserved, so no action is needed to pick up the new look.
