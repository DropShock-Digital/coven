...
- **Cast launcher redesign** (Phases 1–6 on `cast/*` branches): collapses the Coven launcher chrome into a single-prompt surface with a two-lane Commands + Snapshot body, plan/outcome cards for every spell, and a sequential quest flow that hands off between phases with deterministic sub-prompts. The internal visual contract lives in `docs/design/cast-tui-contract.md` and the quest flow in `docs/design/cast-quest-flow.md`. PR #99 is the current review slice (Phase 6 verification + readiness).
